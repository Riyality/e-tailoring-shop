...
public class ReciptController {
	...
	public String displayPage( @RequestParam String contact, Model model ) {
		...
		List<ShirtType> shirtTypes = service.getShirtTypes();
		model.addAttribute( "shirtTypes", shirtTypes );

		List<ShirtPress> shirtPress = service.getShirtPressTypes();
		model.addAttribute( "shirtPressTypes", shirtPress );
	...
	public String addReceipt( @ModelAttribute ReceiptContainer receipt, Model model ) {
		...
		model.addAttribute( "receipt", container );
